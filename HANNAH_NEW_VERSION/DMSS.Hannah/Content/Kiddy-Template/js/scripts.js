...
window.addEventListener( "load", function (){
	...
	canvas_init();
    isotope_init();
    setTimeout(function () { document.getElementById("loadding").style.display = "none";}, 50);
}, false );
...
function AddBookCase(id) {
    jQuery.ajax({
        type: "POST",
        url: '/BookCase/AddToBookcase',
        data: { Book_Id: id },
        success: function (dt) {
            swal({
                title: "Book Added",
...
function RemoveBookCase(id) {
    swal({
        title: "Are you sure?",
        text: "Remove this book from your bookcase?",
        icon: "warning",
        buttons: true,
        dangerMode: true,
    })
...
function sendBookLoan() {

    receiveadd = document.getElementById("receive-add").value;
    receivetime = document.getElementById("receive-time").value;
    returnadd = document.getElementById("return-add").value;
    returntime = document.getElementById("return-time").value;
    note = document.getElementById("note").value;
    var check = document.getElementById("Remember").value;
    if (check == "true") {
        swal({
            ...
            .then((yes) => {
                if (yes) {
                    jQuery.ajax({
                        type: "POST",
                        url: '/BookCase/SendBookLoan',
                        data: { RcAdd: receiveadd, RcTime: receivetime, RtAdd: returnadd, Note: note },
                        success: function (dt) {
                            swal({
                                title: "Submited!",
                                icon: "success",
                                ...
                                .then((willDelete) => {
                                    if (willDelete) {
                                        window.location.href = '/book-loan-histori';
                                    } else {
        ...
        jQuery("#lb-remember").addClass('required');
        jQuery("#lb-remember").focus();
    }
